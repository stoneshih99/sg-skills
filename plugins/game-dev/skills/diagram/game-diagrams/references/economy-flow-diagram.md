...
回答的問題：**資源從哪來（source）、存在哪（池）、花到哪去（sink）、會不會通膨**。是 `../../../planning/game-design/references/gdd-progression-economy.md` 的 source/sink 表的視覺化版本——表格管明細，圖管結構。
...
- 經濟出事時：對照 `../../../tools/game-tooling/references/debug-logging.md` 的經濟事件日誌，在圖上標紅失衡的邊。
