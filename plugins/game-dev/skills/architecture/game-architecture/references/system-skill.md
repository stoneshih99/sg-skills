...
- **髒標記快取**：modifier 增減時標髒，讀取時才重算（不要每幀重算全隊全屬性——見 `../../../tools/game-tooling/references/perf-optimization-playbook.md` 的 dirty flag）。
- **衍生屬性宣告依賴**：攻擊力依賴力量 → 力量髒了連鎖標髒攻擊力。依賴關係成圖，禁環（圖的檢查見 `../../../diagram/game-diagrams/references/system-relation-map.md` 同思路）。
...
- 控制台指令：`buff add/remove <id>`、`skill cast <id>`、列出目標當前 buff 與 modifier 來源（見 `../../../tools/game-tooling/references/debug-console-and-cheats.md`）。
