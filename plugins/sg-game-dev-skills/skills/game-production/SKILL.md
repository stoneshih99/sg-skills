...
description: 遊戲生產流程與資產標準的單一知識入口：流程三件——里程碑（prototype→上線的階段門檻、排程估算、驗收）、建置管線（版控/LFS、CI、RC 凍結與發版）、內容量產（黃金樣本、資產規範、外包交接）；資產標準五件——美術（風格指南、技術規格、視覺可讀性）、音訊（音效、互動音樂、混音響度）、動畫特效、本地化（字串外化、多語版面）、UI/UX（HUD 與選單、無障礙、UI 效能）；平台約束——行動（觸控、發熱降級、電池、中斷、商店審查）與 PC（鍵鼠手把熱插拔、解析度與畫質選項、Steam 慣例）。當「怎麼排時程」「要發版了」「內容做不完」「發外包」「風格不統一」「規格怎麼定」「音量不一致」「特效廉價」「翻譯破版」「HUD 太擠」「要上手機/上 Steam」「手機一玩就燙」「手把提示圖示不對」時使用。引擎中立，偏個人開發與小團隊。
---
...
涵蓋九個域：流程三件（里程碑 / 建置 / 量產）+ 標準五件（美術 / 音訊 / 動畫特效 / 本地化 / UI-UX）+ 平台約束（行動 / PC）。**先查下面的域總表，再進該域的細表挑要讀的 reference。**
...
| 要上手機或 PC/Steam：觸控、發熱、手把、商店審查、畫質選項 | 平台約束 | ↓ Platform |
...
| 何時 | 讀 |
|------|-----|
| 上手機：觸控設計、發熱降級、電池、生命週期中斷、雙商店審查 | `references/platform-mobile-constraints.md` |
| 上 PC/Steam：多輸入熱插拔、顯示碎片化、畫質選單、桌面慣例 | `references/platform-pc-constraints.md` |

## 相鄰 skill

- 企畫層（核心循環 / 範圍 / 風險 / 玩測 / 手感）：`../game-design/`
- 程式架構（資料驅動 / 演算法）：`../game-architecture/`
- 除錯與效能工具（profiler / debug draw / 遙測）：`../game-tooling/`
