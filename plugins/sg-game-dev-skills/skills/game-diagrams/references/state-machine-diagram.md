...
- 角色控制器：實作前先畫，取消窗口與優先權在圖上吵完再寫碼（配 `../../game-design/references/feel-input-responsiveness.md`）。
- AI 行為：Patrol / Chase / Attack / Flee 的切換條件即 AI 的個性所在；debug draw 把當前狀態顯示在頭上（見 `../../game-tooling/references/debug-draw.md`）即可對圖除錯。
